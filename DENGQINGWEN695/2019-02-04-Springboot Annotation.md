...
# springMVC 相关注解
 ...
 ### @RequestMapping
  ...
  params: 指定 request 中必须包含某些参数值是，才让该方法处理。\ headers: 指定 request 中必须包含某些指定的 header 值，才能让该方法处理请求。\ value: 指定请求的实际地址，指定的地址可以是 URI Template 模式\ method: 指定请求的 method 类型， GET、POST、PUT、DELETE 等\ consumes: 指定处理请求的提交内容类型（Content-Type），如 application/json,text/html;\ produces: 指定返回的内容类型，仅当 request 请求头中的 (Accept) 类型中包含该指定类型才返回
